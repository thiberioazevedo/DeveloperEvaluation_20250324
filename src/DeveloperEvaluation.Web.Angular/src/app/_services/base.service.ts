...
export abstract class BaseService<T> {
  ...
  public list(pageNumber: number , pageSize: number, searchText: string, columnOrder: string, asc: boolean, route?: string): Observable<ApiResponseWithData<T>> {
    return <Observable<ApiResponseWithData<T>>>this.httpClient.get(`${this.getEndPoint(route)}?pageNumber=${pageNumber}&pageSize=${pageSize}&searchText=${searchText}&columnOrder=${columnOrder}&asc=${asc}`, { headers: this.ObterHeaderJson() });
  }
  ...
  public put(body: T | T[], route?: string) {
